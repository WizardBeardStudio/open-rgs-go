...
echo "running soak benchmarks (runs=${runs}, benchtime=${benchtime})"
go test ./internal/platform/server -run '^$' -bench '^(BenchmarkLedgerDeposit|BenchmarkWageringPlaceWager)$' -benchmem -count "${runs}" -benchtime "${benchtime}" | tee "${bench_out}"
