...
func resolveEd25519PrivateKey(keyID string) (ed25519.PrivateKey, error) {
	keyRingRaw := strings.TrimSpace(os.Getenv("RGS_VERIFY_EVIDENCE_ATTESTATION_ED25519_PRIVATE_KEYS"))
	if keyRingRaw != "" {
		keyRing := map[string]string{}
		for _, part := range strings.Split(keyRingRaw, ",") {
			p := strings.TrimSpace(part)
	...
	raw := strings.TrimSpace(os.Getenv("RGS_VERIFY_EVIDENCE_ATTESTATION_ED25519_PRIVATE_KEY"))
	if raw == "" {
