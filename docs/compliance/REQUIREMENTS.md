...
- Status: implemented (credential set/rotation, enable/disable controls, bcrypt verification, lockout status/reset operations, failed-attempt lockout policy, actor-mismatch-with-token denials across identity admin operations in gRPC and REST gateway paths, and explicit denied-audit records for identity admin authorization failures)
