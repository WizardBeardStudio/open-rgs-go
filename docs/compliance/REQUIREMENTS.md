...
- Status: implemented (`summary.json` records `summary_validation_log`, `summary_validation_log_sha256`, and attestation file pointers; `verify_evidence` emits signed `attestation.json`/`attestation.sig` with attestation `key_id`; strict validation fails on missing log/artifacts, checksum mismatch, stale `index.txt` entries, missing `manifest.sha256` checksum lines, missing/weak attestation key policy in strict mode, unknown key-id in configured key ring, or attestation signature mismatch; CI enforces strict summary validation, required secret preflight, and explicit artifact presence checks)
