...
- Tests: `internal/platform/server/audit_grpc_test.go`, `internal/platform/server/postgres_integration_test.go` (`TestPostgresAuditChainVerificationPassesForPersistedEvents`, `TestPostgresAuditChainVerificationDetectsTamper`)
- Status: implemented (DB-enabled core services and remote-access outcomes persist audit appends to `audit_events` with hash chaining semantics, `AuditService/ListAuditEvents` reads DB-backed records when configured, and DB chain verification detects tamper/mismatch conditions)
