...
- Code: `internal/platform/auth/grpc_jwt.go`, `internal/platform/auth/jwt.go`, `internal/platform/server/actor_context.go`, `internal/platform/server/identity_grpc.go`, `internal/platform/server/ledger_grpc.go`, `internal/platform/server/wagering_grpc.go`, `internal/platform/server/sessions_grpc.go`, `internal/platform/server/config_grpc.go`, `internal/platform/server/events_grpc.go`, `internal/platform/server/registry_grpc.go`, `internal/platform/server/reporting_grpc.go`, `internal/platform/server/audit_grpc.go`, `internal/platform/server/extensions_grpc.go`
- Tests: `internal/platform/server/identity_grpc_test.go`, `internal/platform/server/identity_gateway_test.go`, `internal/platform/server/ledger_grpc_test.go`, `internal/platform/server/ledger_gateway_test.go`, `internal/platform/server/wagering_grpc_test.go`, `internal/platform/server/wagering_gateway_test.go`, `internal/platform/server/sessions_grpc_test.go`, `internal/platform/server/sessions_gateway_test.go`, `internal/platform/server/config_grpc_test.go`, `internal/platform/server/config_gateway_test.go`, `internal/platform/server/reporting_grpc_test.go`, `internal/platform/server/reporting_gateway_test.go`, `internal/platform/server/audit_grpc_test.go`, `internal/platform/server/registry_events_test.go`, `internal/platform/server/registry_events_gateway_test.go`, `internal/platform/server/extensions_grpc_test.go`, `internal/platform/server/extensions_gateway_test.go`
- Status: implemented (token actor preferred; request actor mismatch denied and asserted across gRPC/REST coverage for identity session/admin APIs with denied-audit assertions, ledger, wagering, sessions, config, reporting, audit, registry/events, and extensions)
...
- Status: implemented (credential set/rotation, enable/disable controls, bcrypt verification, lockout status/reset operations, failed-attempt lockout policy, actor-mismatch-with-token denials across identity admin operations in gRPC and REST gateway paths, explicit denied-audit records for identity admin authorization failures, and denied-audit assertions in gRPC/gateway identity tests)
...
- Tests: `internal/platform/server/identity_grpc_test.go`, `internal/platform/server/identity_gateway_test.go`, `internal/platform/server/postgres_integration_test.go`
- Status: implemented (DB-backed refresh sessions, revoke/rotate workflow, background expiry cleanup, actor-ownership mismatch denials for refresh/logout across gRPC and REST gateway paths, and explicit denied-audit assertions for mismatch paths in both gRPC and REST gateway tests)
...
- Status: implemented (record/list bonus transactions, record/list promotional awards, submit/list system-window events with DB-backed persistence when configured; strict input validation for enums/timestamps/pagination on gRPC+REST paths; strict DB enum decode validation for extension reads; denied and invalid extension paths including missing-actor, invalid-actor-binding, and actor-mismatch-with-token cases emit audit records with explicit denial reasons and are asserted in gRPC/gateway tests; strict production disables in-memory promotions/UI mirrors)
...
- Status: implemented (`summary.json` records `summary_validation_log`, `summary_validation_log_sha256`, `attestation_alg`, and attestation file pointers; `verify_evidence` emits signed `attestation.json`/`attestation.sig` with attestation `alg` + `key_id`; strict validation requires `ed25519` and fails on missing log/artifacts, checksum mismatch, stale `index.txt` entries, missing `manifest.sha256` checksum lines, unknown key-id in configured key ring, missing strict/CI key material, or attestation signature mismatch; CI enforces strict summary validation, required secret preflight, and explicit artifact presence checks. HMAC mode remains local compatibility only and is targeted for retirement at API freeze.)
