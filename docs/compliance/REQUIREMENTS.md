...
- Status: implemented (record/list bonus transactions, record/list promotional awards, submit/list system-window events with DB-backed persistence when configured; strict input validation for enums/timestamps/pagination on gRPC+REST paths; strict DB enum decode validation for extension reads; denied and invalid extension paths including missing-actor and invalid-actor-binding cases emit audit records with explicit denial reasons and are asserted in gRPC/gateway tests; strict production disables in-memory promotions/UI mirrors)
