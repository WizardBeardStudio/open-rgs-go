...
- Status: implemented (DB-enabled core services and remote-access outcomes persist audit appends to `audit_events` with hash chaining semantics, and `AuditService/ListAuditEvents` reads DB-backed records when configured)
