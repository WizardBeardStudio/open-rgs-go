...
- `make verify-evidence-strict` results attached for release commit, including:
  ...
  - `artifacts/verify/<UTC_TIMESTAMP>/attestation.json` (signed attestation payload bound to `summary.json`)
  - `artifacts/verify/<UTC_TIMESTAMP>/attestation.sig` (detached attestation signature; strict/CI requires `ed25519`)
...
- `summary.json` must include validator metadata fields: `summary_validation_status`, `summary_validation_log`, `summary_validation_log_sha256`, `attestation_alg`, `attestation_file`, and `attestation_signature_file`.
