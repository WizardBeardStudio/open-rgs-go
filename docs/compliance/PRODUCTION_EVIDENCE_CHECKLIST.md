...
- `make verify-evidence-strict` results attached for release commit, including:
  ...
  - `artifacts/verify/<UTC_TIMESTAMP>/summary.json` (includes `summary_schema_version`, git commit/ref, pre/post worktree cleanliness metadata, CI run metadata, toolchain/runtime context, dependency + verification-script checksums, command timing metadata, and explicit failure-step attribution)
