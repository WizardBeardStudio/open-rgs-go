...
- `make verify-evidence-strict` results attached for release commit, including:
  ...
  - `artifacts/verify/<UTC_TIMESTAMP>/attestation.sig` (HMAC-SHA256 signature for `attestation.json`)
...
- `summary.json` must include validator metadata fields: `summary_validation_status`, `summary_validation_log`, `summary_validation_log_sha256`, `attestation_file`, and `attestation_signature_file`.
